...
import 'package:chat_app/chat/presentation/widgets/attachment_renderers.dart';
import 'package:chat_app/chat/presentation/widgets/painters.dart';
import 'package:chat_app/chat/presentation/widgets/progress_btn.dart';
...
class _AttachmentPreviewState extends State<AttachmentPreview> {
  bool _doesAttachmentExist() {
    ...
    final fileName = p.basename(widget.message.attachment!.file);
    // final file = File(DeviceStorage.getMediaFilePath(fileName));
    final file = File(widget.message.attachment!.file);

    if (file.existsSync()) {
      widget.message.attachment!.file = file.path;
      return true;
    }
...
class _AttachedVoiceViewerState extends ConsumerState<AttachedVoiceViewer> {
  ...
  void initState() {
    ...
    clientIsSender = widget.message.user!.id == self.id;
    ...
    if (clientIsSender) {
      avatarUrl = self.deviceId;
    } else {
      final other = ref.read(chatControllerProvider.notifier).other;
      avatarUrl = other.deviceId;
    }
  ...
  Widget build(BuildContext context) {
    ...
    final micColor = MessageStatus.seen == MessageStatus.seen
...
class _AttachedDocumentViewerState
  ...
  Future<void> _openDocument() async {
    final attachment = widget.message.attachment!;
    final path = attachment.file;

    try {
      // Open directly in browser (best for PDF, Excel, etc.)
  ...
  Widget build(BuildContext context) {
    ...
    String fileName = p.basename(attachment.file) ?? 'name';
...
class _AttachmentViewerState extends State<AttachmentViewer> {
  ...
  Widget build(BuildContext context) {
    ...
    String formattedTime = formattedTimestamp(widget.message.createAt);
