...
class _AttachmentMessageSenderState
  ...
  Future<void> sendAttachments() async {
    for (var i = 0; i < controllers.length; i++) {
      ...
      await File(attachment.file).copy(
      ...
      ref
          .read(chatControllerProvider.notifier)
          .addAttachment(
              attachment..fileName = p.basename(attachment.file), msgContent)
          .then((value) {
        // ref.read(chatControllerProvider.notifier).sendMessageNoAttachments(
        //     MessageEntity(message: msgContent, attachment: value.id));
      });
